...
class OneClickDemoImport {
  ...
  private $plugin_page_setup = array();
	...
	protected function __construct() {
		// Actions.
		add_action( 'admin_menu', array( $this, 'create_plugin_page' ), 100 );
	...
	public function create_plugin_page() {
		$this->plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
			...
			'menu_title'  => esc_html__( 'Import Demo Content' , 'pt-ocdi' ),
			'capability'  => 'import',
			'menu_slug'   => 'novo_import_demo_content',
		) );
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= '<div class="notice  notice-success"><p>
				<b>That\'s it, all done!</b><br>The demo import has finished.
			</p></div>';
		}
		else {
