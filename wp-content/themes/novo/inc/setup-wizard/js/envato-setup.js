...
var EnvatoWizard = (function ($) {
  ...
  function window_loaded() {
    // init button clicks:
    jQuery('.button-next').on('click', function (e) {
      var loading_button = dtbaker_loading_button(this);
      if (!loading_button) {
        return false;
      }
      if (jQuery(this).data('callback') && typeof callbacks[jQuery(this).data('callback')] != 'undefined') {
        // we have to process a callback before continue with form submission
        callbacks[jQuery(this).data('callback')](this);
        return false;
      } else {
        loading_content();
        return true;
    ...
    jQuery('.theme-presets a').on('click', function (e) {
      e.preventDefault();
      var $ul = jQuery(this).parents('ul').first();
      $ul.find('.current').removeClass('current');
      var $li = jQuery(this).parents('li').first();
      $li.addClass('current');
      var newcolor = jQuery(this).data('style');
      jQuery('#new_style').val(newcolor);
      return false;
    });
  }
  ...
  function PluginManager() {
    ...
    function process_current() {
      if (current_item) {
        ...
        jQuery.post(envato_setup_params.yprm_ajax, {
    ...
    function find_next() {
      ...
      var $li = jQuery('.envato-wizard-plugins li');
      $li.each(function () {
        if (current_item == '' || do_next) {
          current_item = jQuery(this).data('slug');
          $current_node = jQuery(this);
          process_current();
          do_next = false;
        } else if (jQuery(this).data('slug') == current_item) {
          do_next = true;
        }
      });
    ...
    return {
      init: function (btn) {
        jQuery('.envato-wizard-plugins').addClass('installing');
        complete = function () {
          loading_content();
  ...
  function ContentManager() {
    ...
    function find_next() {
      ...
      var $items = jQuery('tr.envato_default_content');
      var $enabled_items = jQuery('tr.envato_default_content input:checked');
      $items.each(function () {
        if (current_item == '' || do_next) {
          current_item = jQuery(this).data('content');
          $current_node = jQuery(this);
          process_current();
          do_next = false;
        } else if (jQuery(this).data('content') == current_item) {
          do_next = true;
        }
      });
    ...
    return {
      init: function (btn) {
        jQuery('.envato-setup-pages').addClass('installing');
        jQuery('.envato-setup-pages').find('input').prop("disabled", true);
        complete = function () {
          loading_content();
          window.location.href = btn.href;
        };
