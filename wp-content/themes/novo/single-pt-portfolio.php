  ...
  if (post_password_required($id)) {
    ...
    if (yprm_get_theme_setting('project_style') != 'horizontal' && (yprm_get_theme_setting('project_style') != 'horizontal-type2' || !(is_array($thumbnails) && !empty($thumbnails) && count($thumbnails) > 0))) { ?>
	      <div class="site-content">
	        ...
	        <div class="post-gallery-masonry row popup-gallery">
	          ...
	          <div class="<?php echo esc_attr($cols_css) ?> popup-item"><a
	              ...
	              data-size="<?php echo wp_get_attachment_image_src($thumb, 'full')[1]; ?>x<?php echo wp_get_attachment_image_src($thumb, 'full')[2]; ?>"><?php echo wp_get_attachment_image($thumb, 'large'); ?></a>
	          </div>
	          <?php } ?>
	        </div>
        ...
        <div class="post-gallery-grid row popup-gallery">
          ...
          <div class="<?php echo esc_attr($cols_css) ?> popup-item"><a
              href="<?php echo esc_url(wp_get_attachment_image_src($thumb, 'full')[0]); ?>"
              data-size="<?php echo wp_get_attachment_image_src($thumb, 'full')[1]; ?>x<?php echo wp_get_attachment_image_src($thumb, 'full')[2]; ?>"
      ...
      <div class="post-bottom">
        <?php if (function_exists('zilla_likes') && yprm_get_theme_setting('project_like') == 'show') {
          echo zilla_likes($id);
        }if ($link = yprm_get_theme_setting('project_gallery_link')) { ?>
        <a href="<?php echo esc_url($link) ?>" class="back-to-main"><i class="fas fa-th"></i></a>
        <?php } if(yprm_get_theme_setting('project_navigation') == 'show') { ?>
          <div class="post-nav">
            <?php if (get_permalink() != $prev) { ?>
            <a href="<?php echo esc_url($prev); ?>"><i class="basic-ui-icon-left-arrow"></i>
              <span><?php echo esc_html__('previous post', 'novo') ?></span></a>
            <?php } ?>
            <?php if (get_permalink() != $next) { ?>
            <a href="<?php echo esc_url($next); ?>"><span><?php echo esc_html__('next post', 'novo') ?></span> <i
                  ...
                  ?>
                    <div class="swiper-slide popup-item">
                      <div class="wrap">
                        ...
                        <a href="<?php echo esc_url($img_array[0]) ?>" data-size="<?php echo esc_attr($img_array[1].'x'.$img_array[2]) ?>"></a>
                      </div>
                    </div>
                  <?php } ?>
                </div>
